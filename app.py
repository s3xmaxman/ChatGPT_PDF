...
def generate_answer(question,openAI_key):
    ...
    prompt += "Instructions: Compose a comprehensive reply to the query using the search results given. "\
              ...
              "answer should be short and concise answer in japanese..\n\nQuery: {question}\nAnswer: "
...
def question_answer(url, file, title_name, question, openAI_key, answer_value):
    if openAI_key.strip()=='':
    ...
    answer_value = generate_answer(question,openAI_key)
    conn = sqlite3.connect('pdf_gpt.db')
    cur = conn.cursor()
    cur.execute("INSERT INTO question_answer(url, title, question, openAI_key, answer) VALUES(?, ?, ?, ?, ?)", (url, title_name, question, openAI_key, answer_value))
    conn.commit()
    conn.close()
    return answer_value
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Group():
            ...
            title_name = gr.Textbox(label='Enter the title of the PDF here')
        ...
        btn.click(question_answer, inputs=[url, file, title_name, question, openAI_key], outputs=[answer])
